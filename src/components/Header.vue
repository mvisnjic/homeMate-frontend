    ...
    <div class="top-0 py-4 px-6 md:px-12 w-full bg-white border-b border-black">
        <div class="flex flex-row items-center justify-between">
            <div class="flex flex-col gap-2">
                ...
                />
                <div class="self-start text-sm" v-if="loggedUsername">
        ...
        >
            <div v-if="authenticated" class="w-2/3 md:w-1/3 h-screen">
                ...
                <router-link to="/">
                    <p
                        :class="$route.name == 'index' ? 'bg-[#aec6cf]' : ''"
                        class="border-b border-black hover:bg-[#9db4bd]"
                    >
                        index
                    </p>
                </router-link>
        ...
        <router-link to="/settings">
          <p :class="$route.name == 'settings' ? 'bg-[#aec6cf]' : ''" class="border-b border-black hover:bg-[#9db4bd]">settings</p>
                ...
                <router-link to="/about">
                    <p
                        :class="$route.name == 'about' ? 'bg-[#aec6cf]' : ''"
                        class="border-b border-black hover:bg-[#9db4bd]"
                    >
                        about
                    </p>
                ...
                <div class="mt-2">
                    <Button class="hover:bg-red-400" @click="logout()"
                        >LOGOUT</Button
                    >
                </div>
            </div>
            <div v-else class="flex flex-col h-screen">
                ...
                <router-link to="/login">
                    <p
                        :class="$route.name == 'login' ? 'bg-[#aec6cf]' : ''"
                        class="border-b border-black hover:bg-[#9db4bd]"
                    >
                        login
                    </p>
